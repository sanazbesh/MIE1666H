...
# size
num_ineq = len(data.weights)
num_var = len(data.prices)
...
for j in range(1, num_var+1):
    prices_dict[j] = data.prices[j-1]

for i in range(1, num_ineq+1):
    capacities_dict[i] = data.capacities[i-1]
    for j in range(1, num_var+1):
        weights_dict[i,j] = data.weights[i-1][j-1]

# Mathematical model
m = pe.ConcreteModel()
# mutable parameters (parametric part of the problem)
m.prices = pe.Param(pe.RangeSet(num_var), initialize=prices_dict)
m.weights = pe.Param(pe.RangeSet(num_ineq), pe.RangeSet(num_var), initialize=weights_dict)
m.capacities = pe.Param(pe.RangeSet(num_ineq), initialize=capacities_dict)
# decision variables
m.x = pe.Var(pe.RangeSet(num_var), domain=pe.Binary)
# objective function 1/2 x^T Q x + p^T x
obj = sum(-1*m.x[j] * m.prices[j]  for j in range(1,num_var+1))
m.obj = pe.Objective(sense=pe.minimize, expr=obj)
# constraints
m.cons = pe.ConstraintList()
for i in range(1,num_ineq+1):
    m.cons.add( sum([m.weights[i,j]*m.x[j] for j in range(1,num_var+1)]) <= m.capacities[i] )
